...
A_FILE = ROOT_DIR / "4.App-ready outputs" / "PS3" / "app_ps3_disc_games.csv"
B_FILE = ROOT_DIR / "6.Tools" / "comparison_input" / "PS3" / "app_ps3_disc_games.csv"
OUTPUT_DIR = ROOT_DIR / "4.App-ready outputs" / "PS3"
OUTPUT_FILE = OUTPUT_DIR / "app_ps3_disc_games_merged.csv"
...
# Ensure schema exists on both sides
...
# Keep only valid PS3 DISC BASE_GAME rows
a_df = a_df[
    (a_df["platform"] == "PS3") &
    (a_df["distribution"] == "DISC") &
    (a_df["content_type"] == "BASE_GAME")
    ].copy()

b_df = b_df[
    (b_df["platform"] == "PS3") &
    (b_df["distribution"] == "DISC") &
    (b_df["content_type"] == "BASE_GAME")
    ].copy()

# Deduplicate each side by title_id
a_df = (
...
# Build final canonical row
result = pd.DataFrame()

result["title_id"] = prefer(merged.get("title_id_a", ""), merged.get("title_id_b", ""))
...
# Prefer A for platform_source unless empty
...
# Structural fields: force correct semantics if missing
...
# Unlock fields usually empty for disc games, but enrich if available
